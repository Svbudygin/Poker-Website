...
def tournament():
    ...
    return render_template('tournament.html', loginin=bool(loginin), coloradmin=bool(loginin in admins))
    ...
    if request.method == "POST":
        ...
        if all_args:
            ...
            cursor.execute(
                ...
                (newName, newPhone, hashed_password, loginin))
            conn.commit()
            cursor.close()
            conn.close()
            return redirect(url_for('games'))
...
def delete_user_data(user_id, input_path):
    conn1 = sqlite3.connect("data/notusers.sql")
    cursor1 = conn1.cursor()
    cursor1.execute(f"DELETE FROM notusers WHERE id = ?", (user_id,))
    conn1.commit()
    cursor1.close()
    conn1.close()
...
def register():
    if request.method == "POST":
        ...
        if filename:
            scheduler.add_job(delete_user_data, 'date', run_date=datetime.now() + timedelta(minutes=7),
                              kwargs={'user_id': user_id[0], 'input_path': folder + "/" + filename})
        else:
            scheduler.add_job(delete_user_data, 'date', run_date=datetime.now() + timedelta(minutes=7),
                              kwargs={'user_id': user_id[0], 'input_path': None})
...
def register2():
    ...
    if request.method == "POST":
        ...
        cursor1.execute('SELECT fullname, phone, email, password, filename, code FROM notusers WHERE id=?',
                        (notuserid,))
...
def nextgame():
    ...
    lst_players = list(filter(lambda x: x, lst_players))
    butmclick = not any(i and i[0] == loginin for i in lst_players)
...
def team():
    ...
    cursor = conn.cursor()
    cursor.execute('SELECT status FROM users WHERE id=?', (loginin,))
    loginin = bool(loginin)
    status = cursor.fetchone()
    if status and status[0].lower() == "guest":
        cursor.close()
        conn.close()
        return redirect(url_for("games"))
